...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use a simple keyword-based scoring approach for speed and reliability
    console.log(`Scoring response for question: ${currentQuestion.id}`);
    
    // Simple scoring function based on keywords
    const scoreResponse = (question: any, userResponse: string) => {
      ...
      } else if (finalScore >= maxPoints * 0.6) {
      ...
      } else if (finalScore >= maxPoints * 0.4) {
    ...
    // Score the response
    const scoreData = scoreResponse(currentQuestion, transcript);
